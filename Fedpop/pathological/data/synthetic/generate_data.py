...
def main():
    ...
    clients_label = [[0, 0] for _ in range(dataset.n_tasks)]
    ...
    for task_id in range(dataset.n_tasks):
        ...
        train_cnt, test_cnt = Counter(y_train), Counter(y_test)

        for label in range(dataset.n_classes):
            clients_label[task_id][label] += train_cnt[label]
            clients_label[task_id][label] += test_cnt[label]

        save_data(x_train, y_train, os.path.join(client_path, "train.pkl"))
        save_data(x_test, y_test, os.path.join(client_path, "test.pkl"))

    clients_label = np.asarray(clients_label)
    similarity = cosine_similarity(clients_label)
