...
def train_fl():
    ...
    print(f"the state of client is {args.stateless_clients}")
    ...
    if args.pfl_algo.lower() == "fedpop":
        args.personalize_on_client = "canonical"
    ...
    # Setup dataloaders
    start_time = time.time()
    train_fed_loader, test_fed_loader = pfl.data.get_federated_dataloader_from_args(args)  # train and test contains the same clients.
    ...
    # Setup PFL optimizer
    # only handle the client optimizer 
    ...
    if args.pfl_algo.lower() == "fedpop":
        # add laplace regularity 
        pfl_args['laplace_coef'] = args.laplace_coef
        # add the similarity file path
        pfl_args['similarity_path'] = f"./dataset_statistics/{args.dataset}_similarity.csv"
        pfl_args['is_sparse'] = False
        if args.dataset.lower() == "stackoverflow":
            pfl_args['is_sparse'] = True
            pfl_args['sparse_path'] = f"./dataset_statistics/{args.dataset}.npz"
            print(f'the value of pfl_args.is_sparse is {pfl_args["is_sparse"]}')
            # TODO: mapping client_id into index.            
    ...
    def _log_test(comm_round, pfl_optim, log_all_clients=False, post_finetune=False):
        ...
        if comm_round >= 200 and 'accuracy|mean' in metrics and 0 <= metrics['accuracy|mean'] < 0.0005:
            ...
            # sys.exit(-1)
        return metrics.get('loss|mean', -1)
    ...
    for comm_round in range(starting_round, args.num_communication_rounds):
        ...
        if (comm_round+1) % args.save_model_every_n_round == 0:
            prev_ckpt_time = time.time()
            round_checkpoint_filename = f'{args.savedir}/checkpoint_{comm_round+1}.pt'
            save_checkpoint(comm_round, avg_loss, pfl_optim, round_checkpoint_filename)
    ...
    final_loss = _log_test(args.num_communication_rounds, pfl_optim, log_all_clients=True, post_finetune=False)
...
def get_pfl_optimizer(pfl_algo, **kwargs):
    ...
    elif pfl_algo.lower() == 'fedpop':
        return pfl.optim.FedPop(**kwargs)
    else:
