...
class Canonical_WordLMTransformer(PFLBaseModel):
    def __init__(self, n_canonical, seq_len, vocab_size, input_dim, attn_hidden_dim, fc_hidden_dim,
                 num_attn_heads, num_layers, interpolate=False, tied_weights=False, dropout_tr=0., dropout_io=0.
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.interpolate:
            outputs = self.linear_interpolation(z, self.decoder)
        else:
            outputs = torch.cat([coder(z).unsqueeze_(-1) for coder in self.decoder], dim=-1)  # mix predictions
            outputs = outputs @ self.canonical_factor
    ...
    def split_server_and_client_params(self, client_mode, layers_to_client, adapter_hidden_dim, dropout=0.0):
        ...
        assert client_mode in ['none', 'tr_layer', 'inp_layer', 'out_layer', 'adapter', 'interpolate', 'finetune', 'canonical']
        is_on_server = None
        ...
        elif self.tied_weights and client_mode in ["canonical"]:
            self.tied_weights = False
            # modify the origin decoder
            self.decoder = nn.ModuleList([nn.Linear(*self.word_embedding.weight.t().shape, bias=False) for _ in range(self.n_canonical)])
            with torch.no_grad():
                self.decoder[0].weight.copy_(self.word_embedding.weight)
                for coder in self.decoder[1:]:
                    nn.init.normal_(coder.weight, 0, .02)
